...
namespace Learun.DataBase.MySqlEx
{
    ...
    public class Database : IDatabase
    {
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@" select  ORDINAL_POSITION f_number, COLUMN_NAME f_column, DATA_TYPE f_datatype, IF (LOCATE('int', column_type) > 0,
   REPLACE(REPLACE (
		REPLACE (column_type, 'int(', ''),
		')',	''),'big','')
   ,0),
 CHARACTER_MAXIMUM_LENGTH) f_length, '' f_identity, IF(COLUMN_KEY='PRI','1','') f_key,
